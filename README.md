...
2. Either push a tag that matches `tauri.conf.json` (`git tag v0.1.0-beta.4 && git push origin v0.1.0-beta.4`) or run **Actions → Release → Run workflow**. The draft tag is always `v` plus the version in `tauri.conf.json` (`v__VERSION__`).
